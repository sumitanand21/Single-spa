...
describe('RCAComponent', () => {
  ...
  it('should check current and page that has changed', () => {
    component.tempClusteringDetections = [{ checkboxdata: false }, { checkboxdata: false }];
    const mockObject = {
      target: {
        checked: true,
      }
    ...
    const result = component.tempClusteringDetections.every(it => it.checkboxdata === true);
    expect(result).toBeTruthy();
  });
  it('should check all checked', () => {
    component.tempClusteringDetections = [{ checkboxdata: false }, { checkboxdata: false }];
    component.isAllChecked();
    const result = component.tempClusteringDetections.every(_ => _.checkboxdata);
  ...
  it('should set on destroy ', () => {
    ...
    expect(component.clusteringService.ClusteringExecution).toEqual(false);
  });
});
