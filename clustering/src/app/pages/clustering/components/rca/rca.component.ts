...
export class RCAComponent implements OnInit, AfterViewChecked, OnDestroy {
  ...
  ngOnInit() {
    this.clusteringService.RCA = true;
  ...
  ngAfterViewChecked() {
    if (this.isTableLoading || this.dataSetDetailsLoading || this.isLoading || this.masterDataLoader) {
      this.clusteringService.disableheader = true;
    } else {
      this.clusteringService.disableheader = false;
    }
  }
  ...
  getDataSetList(loadData?) {
    ...
    this.clusteringService.getClusteringDataSets(JobType).subscribe((res: any) => {
  ...
  loadTableData(enableLoader?) {
    if (this.dataSetSelected && this.correlationFromTimePeriod && this.correlationToTimePeriod) {
      ...
      this.clusteringService.getClusteringModelsTableData(data).subscribe((res: any) => {
  ...
  displayModel(anmdModel) {
    if (!this.checkifObjectExist() || (this.selectedModel &&
      ...
      this.notify.showToastrInfo('Alert', 'Please wait as the current alarm storm is been scheduled');
    }
  }
  // on search
  ...
  ngOnDestroy() {
    this.clusteringService.RCA = false;
    if (this.statusSocket) {
      this.statusSocket.complete();
    }
  }
  ...
  scheduleSequence() {
    if (this.selectedModel) {
      const data = {
        uid: '',
        startTime: this.selectedModel.startTime,
        endTime: this.selectedModel.endTime,
      ...
      this.clusteringService.scheduleClusteringSequence(data).subscribe((res: any) => {
      }, (error) => {
  ...
  scheduleAnomalies() {
    if (this.selectedModel) {
      ...
      this.clusteringService.scheduleClusteringProfiler(data).subscribe((res: any) => {
        if (res.status === 'success') {
  ...
  scheduleSoc(value?) {
    if (value && this.scheduleRes.uid) {
      ...
      this.statusSocket = this.webSocketService.connectToclusterschedulerWebSocket(this.scheduleRes.uid).subscribe((res) => {
  ...
  getRootCauseDetails(scheduleRCA?) {
    if (this.selectedModel) {
      ...
      this.clusteringService.getClusteringProfiler(data).subscribe((res: any) => {
  ...
  openUpsertRcaDialog(action) {
    const name = action === 'clustername' ? 'Edit Cluster Name' : 'Edit Resolution';
    const dialogRef = this.dialog.open(UpsertRcaDetailsComponent, {
      width: action === 'clustername' ? '600px' : '600px',
      maxHeight: '80vh',
      // tslint:disable-next-line:object-literal-shorthand
      data: { name, action, selectedModel: this.selectedModel },
      panelClass: 'anmaly-upser-rca-dialog'
    });


    dialogRef.afterClosed().subscribe(res => {
      if (res) {
        if (res.action === 'clustername') {
          this.selectedModel.RCAObj.clusterName = res.clusterName;

        } else if (res.action === 'resolution') {
          this.selectedModel.resolution = [...res.resolution];
          this.selectedModel.RCAObj.resolution = res.resolution.map(it => it.dispResolution);
        }
    ...
    this.global.opendisplayModal('Do you wish to delete the cluster name',
      'Confirm', 'Delete Cluster Name', true).subscribe(result => {
        if (result === 'save') {
          ...
          this.clusteringService.deleteClusterName(data).subscribe((res: any) => {
